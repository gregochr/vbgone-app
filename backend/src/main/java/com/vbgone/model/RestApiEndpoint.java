...
/**
 * A web API endpoint spotted statically in the source — an ASP.NET Web API action, an ASMX
 * web method, or a WCF-Web ({@code <WebGet>}/{@code <WebInvoke>}) operation. Assure can't wrap
 * these in a safety net yet, so they're reported as a separate, parallel list rather than folded
 * into the readiness buckets.
 *
 ...
 * @param kind      how it was detected: {@code "Web API"}, {@code "ASMX"}, or {@code "WCF"}
