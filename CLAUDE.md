...
- [x] **Assure — endpoint surfacing (static)** — detect and surface the estate's HTTP endpoints as a parallel list (verb, route, path/query params, request/response types): ASP.NET Web API actions, ASMX web methods, and WCF-Web `<WebGet>`/`<WebInvoke>` operations routed by their `UriTemplate` (`#51`). Tolerates VB `_` line-continued attributes (`#50`) and keeps these classes out of the net-ready compile subset (`#49`).
- [ ] **Assure — endpoint behaviour safety net** — characterise the detected endpoints (exercise them, pin their observed behaviour) — **blocked: needs a Windows runner.** Legacy ASP.NET Web API (`System.Web.Http`), ASMX, and WCF-Web are .NET Framework, which only hosts on Windows, so the endpoints can't be *exercised* on a Mac-only setup (detection above is purely static and already works). Unblock via a GitHub Actions `windows-latest` runner, or defer until they're migrated to ASP.NET Core (cross-platform).
